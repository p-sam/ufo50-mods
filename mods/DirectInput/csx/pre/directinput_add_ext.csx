uint _Patch_GMDInput_DefineFunction(UndertaleData utdata, UndertaleExtensionFile file, uint funcIdOffset = 0) {
...
UndertaleExtensionFile extFile = null;
...
if(extFile == null) {
    extFile = new UndertaleExtensionFile()
    {
        Filename = Data.Strings.MakeString(extDllName)
    };
    extension.Files.Add(extFile);
}

extFile.Functions.Clear();
_Patch_GMDInput_DefineFunction(Data, extFile, lastExtFuncId);